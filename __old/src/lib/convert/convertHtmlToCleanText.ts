...
import {toMdast} from 'hast-util-to-mdast/lib'
import {convertMdastToCleanText} from './convertMdastToCleanText'
import type {Element, MdastNodes, State} from 'hast-util-to-mdast/lib/state'
...
export function convertHtmlToCleanText(htmlStr?: string): string | undefined {
  ...
  const mdastTree = toMdast(tree, {
    handlers: {
      br: (state: State, element: Element): void | MdastNodes | MdastNodes[] => {
        return {
          type: 'text',
          value: '\n',
        }
